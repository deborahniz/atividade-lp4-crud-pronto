...
router.get('/', (req, res) => {
    ...
    conexao.query('SELECT * FROM hospital2',(error, results)=>{
        if(error){
            throw error;
        } else {                       
...
router.get('/edit/:id', (req,res)=>{    
    ...
    conexao.query('SELECT * FROM hospital2 WHERE id= $1',[id] , (error, results) => {
        if (error) {
            throw error;
        }else{            
...
router.get('/delete/:id', (req, res) => {
    const id = req.params.id;
    conexao.query('DELETE FROM hospital2 WHERE id = $1',[id], (error, results)=>{
        if(error){
            console.log(error);
        }else{           
            res.redirect('/');         
